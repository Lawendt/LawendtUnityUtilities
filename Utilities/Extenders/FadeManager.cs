...
public class FadeManager : Singleton<FadeManager>
{
    ...
    Dictionary<Image, Coroutine> fadeImg;
    Dictionary<SpriteRenderer, Coroutine> fadeSpr;
    Dictionary<Text, Coroutine> fadeText;
    ...
    FadeManager()
    {
        fadeImg = new Dictionary<Image, Coroutine>();
        fadeSpr = new Dictionary<SpriteRenderer, Coroutine>();
        fadeText = new Dictionary<Text, Coroutine>();
    ...
    #region Image
    IEnumerator _fade(Image render, float from, float to, float second)
    {
        ...
        if (fadeImg.ContainsKey(render))
        {
            fadeImg.Remove(render);
        }
    }

    public void Fade(Image render, float from, float to, float second)
    {
        if (fadeImg.ContainsKey(render))
        {
            Debug.LogWarning("There is a fade coroutine with " + render.name + " in progress.\n Stoping it to start the called");
            StopCoroutine(fadeImg[render]);
            fadeImg.Remove(render);
        }
        fadeImg.Add(render, StartCoroutine(_fade(render, from, to, second)));

    }

    public void StopFade(Image render)
    {
        if (fadeImg.ContainsKey(render))
        {
            fadeImg.Remove(render);
    ...
    #region Text
    IEnumerator _fade(Text render, float from, float to, float second)
    {
        float vel = (to - from) / second;
        float time = second;
        ...
        if (fadeText.ContainsKey(render))
        {
            fadeText.Remove(render);
        }
    }

    public void Fade(Text render, float from, float to, float second)
    {
        if (fadeText.ContainsKey(render))
        {
            Debug.LogWarning("There is a fade coroutine with " + render.name + " in progress.\n Stoping it to start the called");
            StopCoroutine(fadeText[render]);
            fadeText.Remove(render);

        }
        fadeText.Add(render, StartCoroutine(_fade(render, from, to, second)));

    }

    public void StopFade(Text render)
    {
        if (fadeText.ContainsKey(render))
        {
            fadeText.Remove(render);
